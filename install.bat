...
IF ERRORLEVEL 2 (set /p zabbix_server=Input zabbix server ip address: )  
IF "%zabbix_server%"=="=" (set /p zabbix_server=Input zabbix server ip address:  )
...
set /p hostname=input zabbix_agent hostname:  

call :getkey HostMetadata
IF DEFINED HostMetadata ( 
	if "%HostMetadata%"=="=" (ECHO ..) ELSE (echo HostMetadata=%HostMetadata%>>%conf_file%)
)
	
ECHO "Zabbix_server IP address is "%zabbix_server%
ECHO "Zabbix_server IP address is "%zabbix_server% >>%LOGFILE%
ECHO "Zabbix agent Hostname is "%hostname%
ECHO "Zabbix agent Hostname is "%hostname% >>%LOGFILE%
IF DEFINED HostMetadata ( 
	if "%HostMetadata%"=="=" (ECHO "Zabbix agent HostMetadata is "%HostMetadata%
